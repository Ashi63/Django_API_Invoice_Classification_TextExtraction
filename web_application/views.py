...
import os
import numpy as np
from web_application.utils.list_of_fields_question import fields_questions_list_for_invoice,fields_questions_list_for_restobills,fields_questions_list_for_travel
...
def text_extraction_page(request):
    ...
    return render(request,'text_extraction_page.html',{'class_name':class_name,
        ...
        'fields_questions_list_for_bills':fields_questions_list_for_restobills,
        'fields_questions_list_for_travel':fields_questions_list_for_travel})
...
def text_show_files(request):
    ...
    if request.method =="POST":
        ...
        elif class_name == "Resturant_Bills":
            for field in selected_fields:
                #print(fields_questions_list_for_invoice[field])
                selected_question = fields_questions_list_for_restobills[field]
                selected_questions.append(selected_question)
            print("Selected Questions:",selected_questions)
        elif class_name == "Travel":
            for field in selected_fields:
                #print(fields_questions_list_for_invoice[field])
                selected_question = fields_questions_list_for_travel[field]
        ...
        elif class_name == 'Restaurant_Bills':
            for field,question in selected_dict.items():
                user_question = question
                answer = str.upper(generate_result(user_question,image))
                extracted_data[field]=answer
...
def invoice_text_extraction(request):
    if request.method == 'POST':
        ...
        print("Posted Class Name: ",class_name)
